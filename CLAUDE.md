...
- `malyshka-borrow` — у Малышки можно только стрельнуть `50 zł`. Каждый заём: `+50 money`, `-10 mood`, долг (флаг `malyshkaDebt`) растёт. Размер долга показан на панели постоянно (строка «Павінны Малышцы: X zł»). Лимита заёмов нет — только падающее настроение.
