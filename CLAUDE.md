...
- `getFlag(key)` / `setFlag(key, value)` — произвольные флаги прогресса (например, `kranuvkaUsed`). Живут в `state.flags`.
...
- `otec-bar` — Отец-бармен наливает напитки. Каждый напиток даёт `+5 mood`, списывает цену с `money`, изменяет `health`. «Кранувка плиз» бесплатная, `+0.5 health`, одноразовая на партию (флаг `kranuvkaUsed`, не сбрасывается при выходе из сцены).
